...
def download_data(bucket, prefix, down_dir):
    ...
    for obj in objects:
        ...
        if not osp.exists(osp.join(down_dir, prefix)):
            os.makedirs(osp.join(down_dir, prefix))
        bucket_obj.download_file(obj.key, osp.join(down_dir, prefix, filename))

def download_alert(alert_id, download_dir, env='production', input_type='alert_id'):
    ...
    for s3_path in s3_path_list:
        ...
        download_data(parent_s3_bucket, s3_prefix, down_dir=down_dir)
...
def download_alerts(alert_list, download_dir, alert_type='alert_id', env='production'):
    ...
    for alert_id in alert_list:
        try:
            download_alert(alert_id, download_dir, env=env, input_type=alert_type)
...
if __name__ == '__main__':
    ...
    download_alerts(avids, './temp/', alert_type='avid')